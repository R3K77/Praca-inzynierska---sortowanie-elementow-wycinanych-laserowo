...
centroids = load("centroids.csv");

robot = loadrobot('universalUR5e','DataFormat','row');
% robotGP = exampleHelperAddGripper(robot);

% Home Position
homePosition = deg2rad([-15 -126 113 -80 -91 76]);

% Set home position of each joint
robot.Bodies{1, 3}.Joint.HomePosition = homePosition(1);
robot.Bodies{1, 4}.Joint.HomePosition = homePosition(2);
robot.Bodies{1, 5}.Joint.HomePosition = homePosition(3);
robot.Bodies{1, 6}.Joint.HomePosition = homePosition(4);
robot.Bodies{1, 7}.Joint.HomePosition = homePosition(5);
robot.Bodies{1, 8}.Joint.HomePosition = homePosition(6);

% Show robot at home position
f1 = figure;
show(robot,homePosition,'Frames','on','PreservePlot',false,'Collisions','off','Visuals','on');
hold on
...
rotationZ = @(t) [cosd(t) -sind(t) 0;
                  sind(t) cosd(t) 0;
                  0 0 1];

partGT = [offsetX offsetY offsetZ rotation];

% Read all the stl files
stlFiles = dir('meshes/output*.stl');
numFiles = length(stlFiles);

% Create circular shape
radius = 0.1; % radius of the circle
theta = linspace(0, 2*pi, 100); % angles for points on the circle
x = radius * cos(theta); % x-coordinates of the circle points
y = radius * sin(theta); % y-coordinates of the circle points
z = zeros(size(x)); % z-coordinates of the circle points

% Combine the coordinates to create the circular shape
% vertices = [x', y', z'];
% faces = convhull(vertices); % compute the convex hull of the vertices

% % Create the FV structure for the circular shape
...
for i = 1:numFiles
        ...
        % disp(stlFiles(i).name);
        
        % Patch the stl file
        p = patch(gca, 'Faces', FV.ConnectivityList, 'Vertices', ((FV.Points * rotationZ(-partGT(4)))...
                + [partGT(1), partGT(2), partGT(3)]), 'FaceColor', [0.8 0.8 1.0], 'Tag', 'part');
end
...
%     [RefPose,partID,goalZoffset] = exampleHelperSelectPart(partGT);
    RefPose = [centroids(1) centroids(2) partGT(3)];
    partID = p;
    goalZoffset = 0.01;

    % Create goal region based on part position and its reference pose
    goalRegion.ReferencePose = trvec2tform(RefPose);
    goalRegion.ReferencePose(1:3,1:3) = eul2rotm([0 0 0]);
...
%     % Show animation of the interpolated path
    if showAnimation
        ...
        for i = 1 : size(interpConfigurations)
            show(robot,interpConfigurations(i,:),'PreservePlot',false,'Frames','off','Collisions','off','Visuals','on','FastUpdate',true);
            drawnow
            waitfor(rateObj);
        end
    end
    ...
    % endEffectorApproachTransform = getTransform(robot,interpConfigurations(end,:),robot.BodyNames{end});
    % eulerAtEndEffector = tform2eul(endEffectorApproachTransform);

%     % Add part as a collision box with the robot end-effector tool
    robot = exampleHelperAttachPart(robot,-deg2rad(partGT(partID,4))+pi);
